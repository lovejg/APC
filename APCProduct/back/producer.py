# 반드시 kafka가 켜진 상태에서 실행하기
# conda 환경 이용해서 실행하기(collection 이름: connect)
from kafka import KafkaProducer
from json import dumps
from watchdog.observers import Observer # 실시간 변화 감지
from watchdog.events import FileSystemEventHandler # 실시간 변화 감지
import time # delay 주기 위함

class FileWatcher(FileSystemEventHandler): # 파일 변화 감시 함수
    ...
    def on_modified(self, event): # 변화 감지 함수(event 매개변수를 이용)
        if event.src_path == self.filename: # 파일 이름이 일치하면
            with open(self.filename, 'r') as file: # file open
                ...
                for i, line in enumerate(lines[self.last_sent_line_num:], start=self.last_sent_line_num):
                    if line.strip(): # 빈 줄 건너뛰기
                        self.producer.send(self.topic, value=line.strip()) # kafka에 send
                        print(f"Sent to Kafka: {line.strip()}") # send 잘 됐는지 확인하는 출력문
                    self.last_sent_line_num = i + 1 # last_sent_line_num은 data.txt에서의 인덱스 역할을 수행(offset 같은 느낌)

producer = KafkaProducer(acks=0, # 메시지 받은 사람이 메시지를 잘 받았는지 체크하는 옵션(0은 그냥 보내기만 하고 확인 x)
                         compression_type='gzip', # 메시지를 gzip 형태로 압축하여 전송
                         bootstrap_servers=['localhost:9094'], # write 할 kafka broker host
...
observer = Observer() # 감시 설정
event_handler = FileWatcher(path_to_watch, producer, 'FROM_AI') # file, producer, topic name
observer.schedule(event_handler, path='.', recursive=False)
observer.start() # 감시 시작
print("Start listening to port 9094")

# 계속 켜놓을거고, 계속 감지하도록 해야 됨을 고려하여 무한 루프 준비
try:
    while True:
